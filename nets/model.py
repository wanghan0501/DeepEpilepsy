    ...
    def _create_train_model(self):
        with tf.name_scope('train_model'):
            ...
            with tf.name_scope('predictions'):
                ...
                train_one_hot_labels = tf.one_hot(indices=tf.cast(self.labels, tf.int32),
            ...
            with tf.name_scope('metrics'):
                ...
                train_confusion_matrix = tf.confusion_matrix(self.labels, train_classes,
                                                             num_classes=self._config.num_classes)
        ...
        with tf.name_scope('test_model'):
            ...
            with tf.name_scope('metrics'):
                ...
                test_accuracy = tf.reduce_mean(
                    tf.cast(tf.equal(tf.cast(self.labels, tf.int64), test_classes), tf.float32),
                    name='test_accuracy')
                test_confusion_matrix = tf.confusion_matrix(self.labels, test_classes,
                                                            num_classes=self._config.num_classes)
